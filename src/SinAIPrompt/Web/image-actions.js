...
export function createImageActions(frame,{edit,resize}){
  const menu=document.createElement('div');menu.className='image-actions';menu.popover='auto';menu.setAttribute('role','menu');
  menu.innerHTML=[['edit','Edit Image'],['resize','Resize Image'],['fullscreen','View Full Screen'],['external','View Externally']]
  ...
  function open(target){
    ...
    menu.showPopover();
    menu.style.left=Math.max(6,Math.min(innerWidth-menu.offsetWidth-6,host.left+rect.left))+'px';
    menu.style.top=Math.max(host.top+6,Math.min(innerHeight-menu.offsetHeight-6,host.top+rect.top+12))+'px';
  }
