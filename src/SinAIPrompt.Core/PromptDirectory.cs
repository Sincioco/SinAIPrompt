...
public sealed record PromptEntry(string Path, bool IsFolder, string? ParentHtml = null, string? ImageFolder = null)
{
...
public static class PromptDirectory
{
    ...
    public static IReadOnlyList<PromptEntry> Read(string folder, bool showFolders, string sort, string? parentHtml = null, IReadOnlyList<string>? manualOrder = null)
    {
        ...
        return visible.OrderBy(f => f is DirectoryInfo ? 0 : 1)
            ...
            .Select(f => new PromptEntry(f.FullName, f is DirectoryInfo, parentHtml,
                ...
                folders.TryGetValue(System.IO.Path.GetFileNameWithoutExtension(f.Name), out var assets) ? assets.FullName : null)).ToArray();
    }
}
